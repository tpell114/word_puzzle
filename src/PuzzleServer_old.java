...
public class PuzzleServerV2
{
    private ServerSocket serverSocket;

    public PuzzleServerV2(int port)
    {
        try
        {
    ...
    {
        int port = 8080;
        PuzzleServerV2 server = null;
        try
        {
            server = new PuzzleServerV2(port);
            server.listen();
        }
        catch (Exception e)
        {
            System.out.println("Exception caught when trying to establish server with port "+ port);
            System.out.println(e.getMessage());  
        }
    }
    ...
    class WorkerThread implements Runnable
    {
        ...
        private void handleClientReq(String cmdCode, String message, PrintStream out, Socket clientSocket)
        {
            try {
            switch (cmdCode)
            {
                ...
                try
                {
                    ...
                    System.out.println("Contacting word repository to setup level with " + numOfWords + " number of words and a difficulty factor of " + factor); 
                    handleSetupLevel(out, numOfWords, factor);   
                 }
                catch(Exception e)
                {
                    System.out.println("Invalid level set parameters: " + message);
                    sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "Error: Invalid level parameters. Use format 'n:f'");
                }
                break;
                ...
                case ProtocolConstantsV2.CMD_SUBMIT_GUESS:
                    ...
                    try {
                        if (formattedPuzzle == null || hiddenPuzzle == null) {
                            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "ERROR: No active puzzle. Start a new game first.");
                            return;
                        }
                    ...
                    sendMessage(out, ProtocolConstantsV2.CMD_SND_PUZZLE, hiddenPuzzle);
                    }
                 catch (Exception e) {
                    System.out.println("Error processing guess: " + message);
                    sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "ERROR: Invalid guess.");
                }
                break;
                ...
                case ProtocolConstantsV2.CMD_REQ_NEW_GAME:
                //
                break;
    ...
    try {
        if (message == null || message.isEmpty()) {
            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "ERROR: No word provided.");
            break;
        }
        ...
        if (exists == null || exists.equals("ERROR")) {
            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "ERROR: Word repository not responding.");
        } else {
            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "Does the word " + message + " exist?: " + exists);
        }
    } catch (Exception e) {
        System.out.println("Error verifying word with repo: " + message);
        e.printStackTrace();
        sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "ERROR: Word verification failed.");
    }
    break;
                ...
                sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, message + " added?: " + added);
                break;
                ...
                sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, message + " removed?: " + removed);
                break;
        ...
        {
            System.out.println("inside handleSetupLevel");
            
            String stem = contactWordRepository(ProtocolConstantsV2.CMD_GET_STEM_WORD, numOfWords);
            ...
            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, hiddenPuzzle); //client doesnt recieve puzzle cmdsndpuzzle
    ...
    public void run()
    {
        try
        {
            ...
            while ((inputMsg = in.readLine()) != null) 
            {
                ...
                handleClientReq(parts[0], parts[1], out, clientSocket);
            }
    ...
    void handleClientExit(PrintStream out, Socket clientSocket)
    {
        try 
        {
        sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "Terminating connection"); 
           clientSocket.close();
           System.out.println("Client connection closed");
        }
        catch(IOException e)
    ...
    void handleClientSignIn(PrintStream out, String message)
    {
        try
        {
            sendMessage(out, ProtocolConstantsV2.CMD_SND_MISCELLANEOUS, "Welcome " + message + " to the Word Puzzle Game Server!");
        }
        catch(Exception e)
        {
    ...
    {
        return contactWordRepository(ProtocolConstantsV2.CMD_GET_RANDOM_WORD, "dummy");
    }
    ...
    private void sendMessage(PrintStream out, String cmdCode, String message)
    {
        try
        {
            //String fullMessage = String.format("%02X %s", ProtocolConstantsV2.CMD_GET_STEM_WORD, message);
            out.println(cmdCode + " " + message);
    ...
    private static String contactWordRepository(String cmdCode, String message)
    {
        try(DatagramSocket socket = new DatagramSocket())
        {
            ...
            String fullMessage = String.format("%02X %s%s", cmdCode, message, ProtocolConstantsV2.MSG_TERMINATOR);
